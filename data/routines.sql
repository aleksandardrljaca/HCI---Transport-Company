...
--
/*!50003 DROP FUNCTION IF EXISTS `napraviNaduzbenicu` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `napraviNaduzbenicu`(pKlijentId int, pZaposleniId int,pDatumDostave Date) RETURNS tinyint(1)
    READS SQL DATA
BEGIN
    ...
    set vozacSlobodan=(select count(*) from vozac 
                        ...
                        where vozac_vozilo.VOZAC_ID is null);
    set voziloSlobodno=(select count(*) from vozilo
						left outer join vozac_vozilo on vozac_vozilo.VOZILO_ID=vozilo.ID
						where vozac_vozilo.VOZILO_ID is null);
	if (vozacSlobodan > 0 and voziloSlobodno > 0) then
    
		set vozacId=(select min(vozac.ID) from vozac
					left outer join vozac_vozilo on vozac_vozilo.VOZAC_ID=vozac.ID
					where vozac_vozilo.VOZAC_ID is null);
		set voziloId=(select min(vozilo.ID) as ID from vozilo 
						left outer join vozac_vozilo on vozac_vozilo.VOZILO_ID=vozilo.ID
						where vozac_vozilo.VOZILO_ID is null);
...
-- Dump completed on 2023-11-14 12:04:49
